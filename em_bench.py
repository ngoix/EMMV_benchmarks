...
for dat in datasets:
    ...
    s_X_iforest = np.zeros(X_test.shape[0])
    s_X_lof = np.zeros(X_test.shape[0])
    s_X_ocsvm = np.zeros(X_test.shape[0])
    # fit:
    ...
    amax_iforest, axis_t_iforest, em_iforest, AUC_iforest = EM(iforest,
                                                               ...
                                                               averaging=averaging)
    print 'em_lof'
    amax_lof, axis_t_lof, em_lof, AUC_lof = EM(lof, X_train,
                                               X_test, s_X_lof,
                                               max_features=3,
                                               averaging=averaging)

    print 'em_ocsvm'
    amax_ocsvm, axis_t_ocsvm, em_ocsvm, AUC_ocsvm = EM(ocsvm, X_train,
                                                       X_test,
                                                       s_X_ocsvm,
                                                       max_features=3,
                                                       averaging=averaging)
    ...
    print 'mv_lof'
    axis_alpha_lof, mv_lof, AUC_lof = MV(lof, X_train,
                                         X_test, s_X_lof,
                                         max_features=3,
                                         averaging=averaging)

    print 'mv_ocsvm'
    axis_alpha_ocsvm, mv_ocsvm, AUC_ocsvm = MV(ocsvm, X_train,
                                               X_test, s_X_ocsvm,
                                               max_features=3,
                                               averaging=averaging)
    ...
    plt.savefig('mv_em_' + dat + 'supervised_high_3_10')
